<div class="order-wizard-confirmation-module alert fade in">
	<h2 class="order-wizard-confirmation-module-title">{{translate 'Thank you $(0) for shopping with PetShop.co.uk, a Prince’s Trust Funded Business! Your order number is' customer.firstname}}
  {{#if isGuestAndCustomerCenter}}
   <a href="#" data-touchpoint="customercenter" data-hashtag="#/purchases/view/salesorder/{{orderId}}"><u>#{{confirmationNumber}}</u></a>.
  {{else}}
   <u>#{{confirmationNumber}}</u>
  {{/if}}
 </h2>
