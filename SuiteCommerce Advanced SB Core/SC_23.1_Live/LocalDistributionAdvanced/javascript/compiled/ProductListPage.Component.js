...
*/
define("ProductListPage.Component", ["require", "exports", "underscore", "Utils", "jQuery", "SC.VisualComponent", "Facets.Model", "Facets.Translator", "Backbone"], function (require, exports, _, Utils, jQuery, SCVisualComponent, FacetsModel, FacetsTranslator, Backbone) {
    "use strict";
    Object.defineProperty(exports, "__esModule", { value: true });
    exports.ProductListPageComponent = void 0;
    var ProductListPageComponent = function ProductListPageComponentGenerator(application) {
        ...
        var privateComponent = SCVisualComponent.extend({
            ...
            _getOption: function _getOption(options) {
                ...
                var result = _.find(options.list_options, function (list_option) {
            ...
            _getAllOptions: function _getAllOptions(options) {
                var option = this.current_view.translator.getOptionValue(options.option);
                var list_options = Utils.deepCopy(options.list_options);
            ...
            setSorting: function setSorting(options) {
                try {
                    if (this._isViewFromComponent()) {
                        ...
                        var sorting = _.find(sortings, function (sort) {
                        ...
                        if (!sorting) {
                            ...
                            var url_2 = this.current_view.translator
                                .cloneForOption('order', options.sorting)
                                .getUrl();
                            return this.current_view
                                .setEvent('Sorting', options.sorting)
                                .then(function () {
                                Backbone.history.navigate(url_2, { trigger: true });
                            });
                        }
                    }
                    else {
                ...
                try {
                    if (this._isViewFromComponent()) {
                        ...
                        if (!display) {
                            ...
                            var url_3 = this.current_view.translator
                                .cloneForOption('display', options.display)
                                .getUrl();
                            return this.current_view
                                .setEvent('Display', options.display)
                                .then(function () {
                                Backbone.history.navigate(url_3, { trigger: true });
                            });
                        }
                    }
                    else {
            ...
            setPageSize: function setPageSize(options) {
                try {
                    if (this._isViewFromComponent()) {
                        ...
                        var pageSize = _.find(pageSizes, function (pageSize) {
                        ...
                        if (!pageSize) {
                            ...
                            var url_4 = this.current_view.translator
                                .cloneForOption('show', options.pageSize)
                                .getUrl();
                            return this.current_view
                                .setEvent('PageSize', parseInt(options.pageSize, 10))
                                .then(function () {
                                Backbone.history.navigate(url_4, { trigger: true });
                            });
                        }
                    }
                    else {
            ...
            getAllFilters: function getAllFilters() {
                if (this._isViewFromComponent()) {
                    var exclude_1 = _.map((this.current_view.$el
                        .find('[data-view="Facets.FacetedNavigation"]')
                        .data('exclude-facets') || '').split(','), function (facet_id_to_exclude) {
                        return String(facet_id_to_exclude).trim();
                    });
                    var facets = _.filter(this.current_view.model.get('facets'), function (facet) {
                        return !_.contains(exclude_1, facet.id);
                    });
            ...
            setFilters: function setFilters(options) {
                try {
                    if (this._isViewFromComponent()) {
                        ...
                        var translator_1 = this.current_view.translator.cloneWithoutFacets();
                        _.each(filters, function (value, key) {
                            var config = translator_1.getFacetConfig(key);
                            translator_1.facets.push({
                                config: config,
                                id: config.id,
                                url: config.id,
                                value: value,
                        ...
                        var url_5 = translator_1.getUrl();
                        return this.current_view.setEvent('Filters', filters).then(function () {
                            Backbone.history.navigate(url_5, { trigger: true });
                        });
                    }
                    return jQuery.Deferred().reject(false);
                }
            ...
            setSearchText: function setSearchText(options) {
                try {
                    if (this._isViewFromComponent()) {
                        ...
                        var translator = new FacetsTranslator([], translatorOptions, this.current_view.translator.configuration, null);
                        var url_6 = translator.getUrl();
                        return this.current_view.setEvent('SearchText', keywords).then(function () {
                            Backbone.history.navigate(url_6, { trigger: true });
                        });
                    }
                    return jQuery.Deferred().reject(false);
                }
            ...
            getUrl: function getUrl(options) {
                if (this._isViewFromComponent()) {
                    ...
                    var defaultOptions = {
                        currentPage: 1,
                        pageSize: _.find(this.getAllPageSize(), function (option) {
                            return option.isDefault;
                        }).id,
                        display: _.find(this.getAllDisplay(), function (option) {
                            return option.isDefault;
                        }).id,
                        sorting: _.find(this.getAllSorting(), function (option) {
                            return option.isDefault;
                        }).id,
                        searchText: ''
                    };
                    ...
                    var filters = options.filters || [];
                    var translatorFilters_1 = [];
                    _.each(filters, function (value, key) {
                        var config = self_1.current_view.translator.getFacetConfig(key);
                        translatorFilters_1.push({
                            config: config,
                            id: config.id,
                            url: config.id,
                            value: value,
                    ...
                    var translator = new FacetsTranslator(translatorFilters_1, translatorOptions, currentTranslator.configuration, options.category || currentTranslator.categoryUrl);
                    var facetModel = new FacetsModel();
                    var data = translator.getApiParams() || {};
                    var url = facetModel.url();
...
//# sourceMappingURL=ProductListPage.Component.js.map
