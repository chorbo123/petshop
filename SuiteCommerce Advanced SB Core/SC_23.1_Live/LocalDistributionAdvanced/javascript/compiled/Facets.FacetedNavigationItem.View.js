...
*/
define("Facets.FacetedNavigationItem.View", ["require", "exports", "underscore", "facets_faceted_navigation_item.tpl", "Utils", "jQuery", "Configuration", "Backbone.View", "Backbone", "Bootstrap.Slider"], function (require, exports, _, facets_faceted_navigation_item_tpl, Utils, jQuery, Configuration_1, BackboneView, Backbone) {
    "use strict";
    return BackboneView.extend({
        ...
        events: {
            ...
            'keydown div[data-toggle="slider"]': 'handleSliderKeydown',
            'keyup div[data-toggle="slider"]': 'handleSliderKeyup'
        },
        initialize: function () {
        ...
        updateRangeValues: function (e, slider) {
            ...
            var start = _.isFunction(parser) ? parser(slider.values.low, true) : slider.values.low;
            var end = _.isFunction(parser) ? parser(slider.values.high, false) : slider.values.high;
            this.$el
        ...
        updateRangeSelection: function (e, slider) {
            ...
            var facet_values = translator.getFacetValue(this.facetId);
            // available values for that facet
            var values = _.map(this.model.get('values'), function (item) {
        ...
        handleSliderKeydown: function (e) {
            var targetControl = e.target.getAttribute('data-control');
            var slider = this.sliders.data('slider');
            var sliderMinimumValueThumbName = 'low';
            var stopPropagation = true;
            switch (e.key) {
                case 'ArrowLeft':
                case 'ArrowDown':
                    if (targetControl === sliderMinimumValueThumbName) {
                        this.adjustSliderLowValue(slider, -0.1);
                    }
                    else {
                        this.adjustSliderHighValue(slider, -0.1);
                    }
                    break;
                case 'ArrowRight':
                case 'ArrowUp':
                ...
                case 'PageDown':
                    if (targetControl === sliderMinimumValueThumbName) {
                        this.adjustSliderLowValue(slider, -0.2);
                    }
                    else {
                        this.adjustSliderHighValue(slider, -0.2);
                    }
                    break;
                case 'PageUp':
                    if (targetControl === sliderMinimumValueThumbName) {
                ...
                case 'Home':
                    if (targetControl === sliderMinimumValueThumbName) {
                        slider.values.low = slider.values.min;
                    }
                    else {
                        slider.values.high = slider.values.min;
                    }
                    break;
                case 'End':
                    if (targetControl === sliderMinimumValueThumbName) {
                        slider.values.low = slider.values.max;
                    }
                    else {
                        slider.values.high = slider.values.max;
                    }
                    break;
                default:
            ...
            if (stopPropagation) {
                e.preventDefault();
                e.stopPropagation();
                this.updateRangeValues(e, slider);
                slider.slideControls();
                slider.resizeBar();
            }
        },
        adjustSliderLowValue: function (slider, value) {
            slider.values.low += slider.values.low > 0 ? slider.values.low * value : 10 * value;
            if (slider.values.low < slider.values.min) {
                slider.values.low = slider.values.min;
            }
            if (slider.values.low > slider.values.high) {
                slider.values.low = slider.values.high;
            }
        },
        adjustSliderHighValue: function (slider, value) {
            slider.values.high += slider.values.high > 0 ? slider.values.high * value : 10 * value;
            if (slider.values.high > slider.values.max) {
                slider.values.high = slider.values.max;
            }
            if (slider.values.high < slider.values.low) {
                slider.values.high = slider.values.low;
            }
        },
        handleSliderKeyup: function (e) {
            var slider = this.sliders.data('slider');
            switch (e.key) {
                case 'ArrowLeft':
                case 'ArrowDown':
                ...
                case 'Home':
                case 'End':
                    this.updateRangeSelection(e, slider);
                    break;
                default:
                    break;
            }
        ...
        getContext: function () {
            ...
            if (facet_config.behavior !== 'range') {
                ...
                _(display_values).each(function (value) {
                    value.isLightColor = _.contains(Configuration_1.Configuration.get('layout.lightColors', []), value.label);
                });
...
//# sourceMappingURL=Facets.FacetedNavigationItem.View.js.map
