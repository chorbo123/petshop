    ...
    var ApplicationLayout = /** @class */ (function (_super) {
        ...
        function ApplicationLayout(application) {
            ...
            _this.events = {
                ...
                'click [data-action="skip-to-main-content"]': 'goToMainContentSection'
            };
        ...
        ApplicationLayout.prototype.goToMainContentSection = function (e) {
            e.preventDefault();
            e.stopImmediatePropagation();
            var targetElement = jQuery(e.target.hash.replace(/##/, '#'));
            if (targetElement) {
        ...
        ApplicationLayout.prototype.addModalListeners = function (view, current_view) {
            ...
            // We added this event listener for the sake of accessibility to catch the escape key
            // this will cause the modal to close without opening SMT
            view.$containerModal.on('keyup', function (e) {
                if (e.key === 'Escape') {
                    e.stopImmediatePropagation();
                }
            });
        ...
        ApplicationLayout.prototype._showModalInDOM = function (view, options, currentModalView, promise) {
            ...
            view.$containerModal.modal(__assign(__assign({}, options.modalOptions), { focus: true }));
        ...
        ApplicationLayout.prototype.ajaxError = function (jqXhr, options, errorText) {
            ...
            if (!jqXhr.preventDefault) {
                ...
                if (options.type === 'GET' && options.killerId) {
                    ...
                    else if (intStatus >= 400 && intStatus < 500) {
                        // Not Found error, we show that error
                        this.notFound();
                    }
                    else {
