...
*/
define("LiveOrder.Model", ["require", "exports", "underscore", "Loggers", "Utils", "InstrumentationAddToCart", "jQuery", "Configuration", "AjaxRequestsKiller", "Profile.Model", "Transaction.Model", "Transaction.Shipmethod.Collection", "LiveOrder.Line.Collection", "LiveOrder.Line.Model", "Session", "Singleton", "Tracker"], function (require, exports, _, Loggers_1, Utils, InstrumentationAddToCart, jQuery, Configuration_1, AjaxRequestsKiller_1, Profile_Model_1, TransactionModel, TransactionShipmethodCollection, LiveOrderLineCollection, LiveOrderLineModel, Session, Singleton, Tracker) {
    "use strict";
    ...
    var LiveOrderModel = TransactionModel.extend({
        ...
        initialize: function initialize(attributes) {
            ...
            if (SC.ENVIRONMENT.customFieldsMetadata &&
        ...
        addLines: function addLines(lines) {
            ...
            return this.cancelableTrigger('before:LiveOrder.addLines', lines).then(function () {
                ...
                var promise = lines.sync('create', lines, {
                ...
                promise.done(function (attributes) {
                    ...
                    InstrumentationAddToCart.addOperationIdToCartLine(self.get('lines').models, lines);
                ...
                return line
                    ...
                    try {
                        ...
                        if (error_details && error_details.status === 'LINE_ROLLBACK') {
                            var line_1 = self.get('lines').get(error_details.oldLineId);
                            if (line_1) {
                                line_1.set('internalid', error_details.newLineId);
                                self.trigger('LINE_ROLLBACK', line_1);
                            }
                        }
                    }
                });
        ...
        removeLine: function removeLine(line) {
            ...
            var actionId = loggers.start('Remove from Cart');
            return this.cancelableTrigger('before:LiveOrder.removeLine', line).then(function () {
                var item = InstrumentationAddToCart.itemToTrack(line);
                return line
                    .sync('delete', line, {
                    killerId: AjaxRequestsKiller_1.AjaxRequestsKiller.getKillerId(),
                    validate: false
                    ...
                    Tracker.getInstance().trackRemoveFromCart(line, item.itemQuantity);
        ...
        submit: function submit() {
            var loggers = Loggers_1.Loggers.getLogger();
            var actionId = loggers.start('Place Order');
            var self = this;
            return this.cancelableTrigger('before:LiveOrder.submit').then(function () {
                ...
                var paypal = self.get('paymentmethods').findWhere({ type: 'paypal' });
                ...
                var beforeActionOperationIds = _.clone(self.getOperationIds());
                ...
                var cartLines = lines ? "" + lines.length : '';
                var promoCodes = self.get('promocodes').filter(function (code) { return code.isvalid; });
                self.set('promocodes', promoCodes);
                return self
                    .save()
                    .done(function (order) {
                    var lastOperationIdIndex = self.getOperationIds().length;
                    if (beforeActionOperationIds && beforeActionOperationIds.length > 0) {
                        lastOperationIdIndex = beforeActionOperationIds.length;
                    }
                    var orderOperationIds = self.getLatestOperationIds(lastOperationIdIndex);
                    loggers.end(actionId, {
                    ...
                    var suiteTaxEnabled = Configuration_1.Configuration.get('siteSettings.isSuiteTaxEnabled');
                    if (suiteTaxEnabled) {
                        jQuery
                            .get(Utils.getAbsoluteUrl("services/SuiteTax.ss?internalid=" + order.confirmation.internalid + "&n=" + SC.ENVIRONMENT.siteSettings.siteid + "&domain=" + SC.ENVIRONMENT.shoppingDomain, true))
        ...
        save: function save() {
            if (this.get('confirmation') &&
                this.get('confirmation').statuscode !== 'error' &&
                this.get('confirmation').get('internalid')) {
                return jQuery.Deferred().resolve();
            }
        ...
        shippingAddressIsRequired: function shippingAddressIsRequired() {
            return (this.getIfThereAreDeliverableItems() &&
                Configuration_1.Configuration.get('siteSettings.requireshippinginformation', 'F') === 'T');
        },
        ...
        addPromotion: function addPromotion(promocode) {
            ...
            return this.cancelableTrigger('before:LiveOrder.addPromotion', {
                ...
                var new_promocodes = pre_promocodes.concat([{ code: promocode }]);
                return self
        ...
        setShipMethod: function setShipMethod(shipMethodId) {
            var _this = this;
            return this.cancelableTrigger('before:LiveOrder.setShipMethod', shipMethodId).then(function () {
                _this.set('shipmethod', shipMethodId);
                return _this.save().then(function () {
                    var shippingMethod = _.find(_this.get('shipmethods').toJSON(), function (shipMethod) { return shipMethod.internalid === shipMethodId; });
                    _this.cancelableTrigger('after:LiveOrder.setShipMethod', shippingMethod);
        ...
        setPurchaseOrderNumber: function setPurchaseOrderNumber(purchaseNumber) {
            var _this = this;
            return this.cancelableTrigger('before:LiveOrder.setPurchaseOrderNumber', purchaseNumber).then(function () {
                _this.set('purchasenumber', purchaseNumber);
                _this.cancelableTrigger('after:LiveOrder.setPurchaseOrderNumber', purchaseNumber);
            });
        },
        // @method trackItemQuantityChangeEvent
        // @param {Transaction.Line.Model} line
        // @param {Number} difference
        // If difference is positive then the AddToCart event will be fired.
        // If it´s negative then the remove_from_cart event will be fired.
        trackItemQuantityChangeEvent: function trackItemQuantityChangeEvent(line, difference) {
            if (difference > 0) {
                Tracker.getInstance().trackAddToCartSpecificQuantity(line, Math.abs(difference));
            }
            else if (difference < 0) {
                Tracker.getInstance().trackRemoveFromCart(line, Math.abs(difference));
            }
        }
    }, ClassProperties);
...
//# sourceMappingURL=LiveOrder.Model.js.map
