...
define("Utils", ["require", "exports", "underscore", "String.format", "jQuery", "Backbone", "Backbone.Model"], function (require, exports, _, String_format_1, jQuery, Backbone, BackboneModel) {
        ...
        try {
            switch (errorCode) {
                case ITEM_NO_LONGER_AVAILABLE: {
                    return this.translate('Unfortunately this item is not longer available. Please <a href="#" data-touchpoint="viewcart"">go back</a> and review the cart');
                }
                default: {
    ...
    function initBxSlider($element, options) {
        ...
        // bxSlider keeps the original controls in the DOM even when using custom ones
        // these original controls are not needed and are breaking the accesibility
        if (options.prevText) {
            jQuery('.bx-prev').remove();
        }
        if (options.nextText) {
            jQuery('.bx-next').remove();
        }
        return $element;
    }
