...
*/
define("OrderWizard.Module.MultiShipTo.Shipmethod", ["require", "exports", "underscore", "order_wizard_msr_shipmethod_module.tpl", "Utils", "jQuery", "Wizard.StepModule", "Backbone.CollectionView", "OrderWizard.Module.MultiShipTo.Shipmethod.Package.View", "Transaction.Shipmethod.Collection", "Tracker"], function (require, exports, _, order_wizard_msr_shipmethod_module_tpl, Utils, jQuery, Wizard_StepModule_1, BackboneCollectionView, OrderWizardModuleMultiShipToShipmethodPackageView, TransactionShipmethodCollection, Tracker) {
    "use strict";
    return Wizard_StepModule_1.WizardStepModule.extend({
        ...
        shippingAddressChange: function (updated_address) {
            ...
            if (addrress_model) {
                ...
                var change_country = addrress_model.get('country') !== updated_address.get('country');
                var change_state = addrress_model.get('state') !== updated_address.get('state');
        ...
        getDefaultCollapseItem: function () {
            var sca = this.wizard.application.getConfig().sca;
        ...
        changeExpandedState: function (e) {
            var selected_address_id = jQuery(e.target)
        ...
        isActive: function () {
            ...
            var result = this.wizard.model.get('ismultishipto') &&
                this.wizard.model.getIfThereAreDeliverableItems() &&
                _.keys(shipping_methods).length;
            if (result) {
                var all_item_has_shipping_address_selected = this.wizard.model
                    .get('lines')
                    .all(function (line) {
                ...
                if (this.is_read_only) {
                    var all_item_has_shipping_method_selected = this.wizard.model
                        .get('lines')
                        .all(function (line) {
        ...
        selectDeliveryMethodComboHandler: function (e) {
            ...
            var selected_delivery_mehtod_id = $selected_combo.val();
            var selected_address_id = $selected_combo.data('address-id');
        ...
        selectDeliveryMethod: function (selected_delivery_method_id, selected_address_id, save_changes) {
            if (this.canSaveChanges()) {
                ...
                var delivery_methods = this.wizard.model.get('multishipmethods')[selected_address_id];
        ...
        saveChanges: function (submit_server) {
            ...
            if (this.canSaveChanges()) {
                ...
                var shipping_methods_1 = this.wizard.model.get('multishipmethods');
                lines.each(function (line) {
                    var ship_address = line.get('shipaddress');
                    if (ship_address) {
                        var selected_ship_method = shipping_methods_1[ship_address].findWhere({
                            check: true
                        });
                ...
                if (submit_server) {
                    ...
                    this.submit_promise.always(function () {
                        var shipmethods = [];
                        _.each(shipping_methods_1, function (shipmethodsCollection) {
                            var shipmethod = shipmethodsCollection.findWhere({
                                check: true
                            });
                            if (shipmethod) {
        ...
        getPackages: function () {
            if (this.isActive() && this.state === 'present') {
                var result_1 = [];
                var self_1 = this;
                var items_per_address_1 = this.wizard.model.get('lines').groupBy(function (line) {
                    return line.get('shipaddress');
                });
                var addresses_1 = this.profile.get('addresses'); // Get the addresses from profile because when updating just the user's address are updated
                var selected_delivery_methods_1 = [];
                var shipping_methods_2 = this.wizard.model.get('multishipmethods');
                _.each(_.keys(items_per_address_1), function (address_id) {
                    if (address_id === 'null' || !shipping_methods_2[address_id]) {
                        return;
                    }
                    if (!self_1.is_read_only) {
                        var item_set = _.find(items_per_address_1[address_id], function (item) {
                        ...
                        if (item_set) {
                            shipping_methods_2[address_id]
                                .get(item_set.get('shipmethod'))
                                .set('check', true);
                        }
                        selected_delivery_methods_1 = shipping_methods_2[address_id];
                    }
                    else {
                        // Get THE selected ship method for the entire package
                        var ship_method_group_id = _.first(items_per_address_1[address_id]).get('shipmethod');
                        var ship_method = shipping_methods_2[address_id].get(ship_method_group_id);
                        if (ship_method) {
                            ship_method.set('check', true);
                            selected_delivery_methods_1 = new TransactionShipmethodCollection([
                                ship_method
                            ]);
                        }
                    }
                    result_1.push({
                        lines: items_per_address_1[address_id],
                        address: addresses_1.get(address_id) ||
                            self_1.wizard.model.get('addresses').get(address_id),
                        collapse_elements: self_1.packages_items_collapsed[address_id],
                        summary: {},
                        deliveryMethods: selected_delivery_methods_1
                    });
                });
                return result_1;
            }
...
//# sourceMappingURL=OrderWizard.Module.MultiShipTo.Shipmethod.js.map
