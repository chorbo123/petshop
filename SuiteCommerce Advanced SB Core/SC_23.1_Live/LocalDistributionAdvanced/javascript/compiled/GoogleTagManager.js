...
define("GoogleTagManager", ["require", "exports", "underscore", "Utils", "jQuery", "Configuration", "Tracker", "GoogleTagManager.NavigationHelper.Plugins.Standard", "GoogleTagManager.Model", "Backbone", "js.cookie"], function (require, exports, _, Utils, jQuery, Configuration_1, Tracker, GoogleTagManagerNavigationHelper, GoogleTagManagerModel, Backbone, Cookies) {
    ...
    var GoogleTagManager = {
        ...
        // @return {Void}
        trackAddPaymentInfo: function (_a) {
            var cart = _a.cart, paymentType = _a.paymentType;
            if (cart) {
                ...
                cart.get('lines').each(function (model, index) {
                    ...
                    eventData_2.ecommerce.items.push({
                        item_id: item.getSku(),
                ...
                // Triggers a Backbone.Event so others can subscribe to this event and add/replace
                // data before being sent to Google Tag Manager
                Tracker.trigger(eventName, eventData_2);
                this.pushData(eventData_2);
            }
            return this;
        },
        // @method trackAddShippingInfo
        trackAddShippingInfo: function (_a) {
            var shipmethods = _a.shipmethods, cart = _a.cart;
            if (shipmethods.length) {
                ...
                lines.each(function (line) {
                    ...
                    eventData_3.ecommerce.items.push({
                        item_name: item.get('_name'),
                ...
                this.pushData(eventData_3);
            }
            return this;
        },
        trackRefund: function (_a) {
            var _this = this;
            var requestReturn = _a.requestReturn;
            var summary = requestReturn.summary, lines = requestReturn.lines;
            ...
            lines.forEach(function (line) {
                ...
                eventData.ecommerce.items.push({
                    ...
                    index: line.index,
                    item_category: _this.getCategory(),
        ...
        trackCartUpdate: function (items) {
            if (items.length) {
                ...
                // Triggers a Backbone.Event so others can subscribe to this event and add/replace
                // data before being sent to Google Tag Manager
                Tracker.trigger(eventName, eventData_4, items);
                this.pushData(eventData_4);
            }
            return this;
        },
        ...
        loadScript: function () {
            return jQuery.getScript("//www.googletagmanager.com/gtm.js?id=" + this.configuration.id + "&l=" + this.configuration.dataLayerName);
        },
