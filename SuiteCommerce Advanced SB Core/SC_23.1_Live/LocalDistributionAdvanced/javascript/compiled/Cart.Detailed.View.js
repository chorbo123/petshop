    ...
    return BackboneView.extend({
        ...
        updateItemQuantity: function updateItemQuantity(e) {
            ...
            if (!validInput || options.quantity) {
                var newQuantity = parseInt(options.quantity, 10);
                var item = line.get('item');
                var minQuantity = item
                    ? item.get('_minimumQuantity', true)
                    : line.get('minimumquantity');
                var maxQuantity = item
                    ? item.get('_maximumQuantity', true)
                    : line.get('maximumquantity');
                var currentQuantity_1 = parseInt(line.get('quantity'), 10);
                var newValidQuantity = newQuantity >= minQuantity ? newQuantity : minQuantity;
                newValidQuantity =
                    !!maxQuantity && newQuantity > maxQuantity
                        ...
                        : newQuantity > minQuantity
                            ? newQuantity
                            : minQuantity;
                $input.val(newValidQuantity);
                if (newQuantity !== currentQuantity_1) {
                    $input.val(newValidQuantity).prop('disabled', true);
                    line.set('quantity', newValidQuantity);
                    var invalid = line.validate();
                    if (!invalid) {
                        var difference = line.get('quantity') - currentQuantity_1;
                        this.model.trackItemQuantityChangeEvent(line, difference);
                        ...
                        update_promise
                            ...
                            line.set('quantity', currentQuantity_1);
                            self.render();
                        })
                            .always(function () {
