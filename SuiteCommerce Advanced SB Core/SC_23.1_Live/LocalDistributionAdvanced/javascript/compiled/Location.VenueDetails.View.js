...
*/
define("Location.VenueDetails.View", ["require", "exports", "underscore", "Utils", "locator_venue_details.tpl", "Backbone.View"], function (require, exports, _, Utils, locator_venue_details_tpl, BackboneView) {
    "use strict";
    var LocationVenueDetailsView = BackboneView.extend({
        ...
        setOpeningHours: function setOpeningHours() {
            ...
            var opening_hours_str_list = [];
            _.each(opening_hours, function (row) {
                var hours = row.starttime + " " + Utils.translate('to').toLowerCase() + " " + row.endtime;
                var days_list = [
                    row.monday === 'T',
                    row.tuesday === 'T',
                    row.wednesday === 'T',
                ...
                var days = '';
                var sequence_days = [];
                var discontinuous_days = [];
                var sequence_active = false;
                var sequence_finish = false;
                ...
                if (sequence_active) {
                    days = sequence_days[0] + " " + Utils.translate('to').toLowerCase() + " " + sequence_days[sequence_days.length - 1];
                }
                _.each(discontinuous_days, function (discontinuous_day, key) {
                    if (key === 0 && !sequence_active) {
                        ...
                        days = days + " " + Utils.translate('and', 'This "and" is used when enumerating days like: Monday, Wednesday and Friday. Prior to the last day enumerated we add the "and" word') + " " + discontinuous_day;
                    }
                    else {
    ...
    return LocationVenueDetailsView;
});

//# sourceMappingURL=Location.VenueDetails.View.js.map
