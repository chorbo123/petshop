...
define("Item.Model", ["require", "exports", "underscore", "Utils", "Configuration", "MasterOptionsHelper", "Profile.Model", "UrlHelper", "Backbone.CachedModel", "Session", "Item.Option.Collection"], function (require, exports, _, Utils, Configuration_1, MasterOptionsHelper_1, Profile_Model_1, UrlHelper_1, BackboneCachedModel, Session, ItemOptionCollection) {
    ...
    var ItemModel = BackboneCachedModel.extend({
        ...
        getStrikeThroughPrice: function getStrikeThroughPrice() {
            if (!this._showStrikeThrough()) {
                return null;
            }
            var defaultPriceLevel = SC.CONFIGURATION.priceLevel.default;
            var defaultItemPrice = this.get("" + defaultPriceLevel);
            var defaultItemPriceFormatted = this.get(defaultPriceLevel + "_formatted");
            var matrixChildren = this.get('_matrixChilds') ? this.get('_matrixChilds').models : [];
            if (matrixChildren.length) {
                return this._getStrikeThroughForMatrixItems(matrixChildren);
            }
            var itemPrice = this.getPrice();
            return itemPrice.price < defaultItemPrice ? defaultItemPriceFormatted : null;
        },
        _showStrikeThrough: function showStrikeThrough() {
        ...
        getComparePrice: function getComparePrice(price, strikeThroughPrice) {
            var comparePrice = '';
        ...
        _getStrikeThroughForMatrixItems: function _getStrikeThroughForMatrixItems(matrixChildren) {
            var childrenPrices = [];
            var defaultPriceLevel = SC.CONFIGURATION.priceLevel.default;
            var defaultItemPrice = this.get("" + defaultPriceLevel);
            _.each(matrixChildren, function (child) {
                childrenPrices.push({
                    price: child.get("" + defaultPriceLevel),
                    priceFormatted: child.get(defaultPriceLevel + "_formatted")
                });
            });
            var childrenPricesValues = _.pluck(childrenPrices, 'price');
            var minValue = _.min(childrenPricesValues);
            var maxValue = _.max(childrenPricesValues);
            var minPrice = _.where(childrenPrices, { price: minValue })[0];
            var maxPrice = _.where(childrenPrices, { price: maxValue })[0];
