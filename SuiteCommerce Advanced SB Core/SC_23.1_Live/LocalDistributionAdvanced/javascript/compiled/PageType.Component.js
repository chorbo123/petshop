...
*/
define("PageType.Component", ["require", "exports", "underscore", "Loggers", "PageType.Collection", "Utils", "jQuery", "Configuration", "PageType.Settings.Model", "SC.BaseComponent", "PageType.Router", "Backbone"], function (require, exports, _, Loggers_1, PageType_Collection_1, Utils, jQuery, Configuration_1, PageType_Settings_Model_1, SC_BaseComponent_1, PageTypeRouter, Backbone) {
    "use strict";
    ...
    return function PageTypeComponentGenerator(application) {
        ...
        var PageTypeComponent = SC_BaseComponent_1.SCBaseComponent.extend({
            ...
            registerPageType: function registerPageType(pagetype) {
                ...
                function callback() {
                    ...
                    if (pagetype.view) {
                        ...
                        var page = pages[url];
                        var options_1 = _.extend({}, pagetype.options || {}, {
                            application: application,
                        ...
                        if (page) {
                            ...
                            var lazySettingsLoad = _.map(Configuration_1.Configuration.get('cms.lazySettingsLoad', []), function (scriptId) { return scriptId.toUpperCase(); });
                            if (page.get('customrecordscriptid') &&
                                lazySettingsLoad.indexOf(page.get('customrecordscriptid').toUpperCase()) !== -1) {
                                var model = new PageType_Settings_Model_1.PageTypeSettingsModel({
                                    internalId: page.get('customrecorddata'),
                                    recName: page.get('customrecordscriptid')
                                });
                                model.fetch().done(function (fields) {
                                    options_1.pageInfo.fields = fields;
                                    view = new pagetype.view(options_1);
                                    PageTypeComponent._CmsViewPromisesDone({
                                        view: view,
                                        pageType: pagetype,
                                        loggers: loggers
                                    });
            ...
            _CmsViewPromisesDone: function (doneData) {
                var data = {
                    view: doneData.view,
                    page_type: doneData.pageType.name
                };
                PageTypeComponent._CmsViewPromises(data).done(function () {
                    doneData.view._pagetype = true;
                    var showContentPromise = doneData.view.showContent();
                    showContentPromise &&
                        showContentPromise.done(function (result) {
                            var cctComponent = application.getComponent('CMS');
                            cctComponent.addContents();
                            var options = {};
                            if (doneData.pageType.name === 'ProductDetails.Full.View') {
                                checkNavigationItem(options, result);
                            }
                            else if (doneData.pageType.name === 'facet-browse') {
                                checkNavigationSearchResults(options, result);
                            }
                            if (doneData.view.inModal) {
                                options.displayedInModal = true;
                            }
                            doneData.loggers.endLast('Navigation', options);
                            if (Configuration_1.Configuration.get('cms.useCMS')) {
