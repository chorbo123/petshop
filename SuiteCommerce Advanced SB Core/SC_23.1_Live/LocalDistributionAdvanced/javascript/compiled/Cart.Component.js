...
define("Cart.Component", ["require", "exports", "underscore", "jQuery", "ICart.Component", "LiveOrder.Model", "LiveOrder.Line.Model", "Transaction.Paymentmethod.Model", "Backbone", "Utils", "Cart.Confirmation.Helpers"], function (require, exports, _, jQuery, ICartComponent, LiveOrderModel, LiveOrderLineModel, TransactionPaymentmethodModel, Backbone, Utils, CartConfirmationHelpers) {
    ...
    return function CartComponentGenerator(application) {
                ...
                try {
                    ...
                    this.serialize = this.serialize.pipe(function () {
                        ...
                        try {
                            ...
                            var oldLines_1 = live_order_model.get('lines').toJSON();
                            live_order_model
                                .addLines(lines)
                                .done(function (result) {
                                var oldLinesById = _.indexBy(oldLines_1, 'internalid');
                                var newLines = _.filter(result.lines, function (_a) {
                                    var internalid = _a.internalid, quantity = _a.quantity;
                                    return (!oldLinesById[internalid] ||
                                        oldLinesById[internalid].quantity !== quantity);
                                });
                                deferred_2.resolve(_.pluck(newLines, 'internalid'));
                            })
                                .fail(function (error) {
        ...
        live_order_model.cancelableOn('after:LiveOrder.addLines', function (oldLines, currentLines) {
            if (!cart_component.hasListeners('afterAddLine')) {
                return jQuery.Deferred().resolve();
            }
            var oldLinesById = _.indexBy(oldLines, 'internalid');
            var newLines = _.filter(currentLines, function (_a) {
                var internalid = _a.internalid, quantity = _a.quantity;
                return !oldLinesById[internalid] || oldLinesById[internalid].quantity !== quantity;
            });
            var linesDeferred = _.map(newLines, function (line) {
            ...
            return jQuery.when.apply(jQuery, linesDeferred);
        });
