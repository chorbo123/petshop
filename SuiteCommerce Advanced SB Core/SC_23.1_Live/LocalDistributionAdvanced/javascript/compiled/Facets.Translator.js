...
*/
define("Facets.Translator", ["require", "exports", "underscore", "jQuery", "Utils", "Configuration", "UrlHelper", "Backbone"], function (require, exports, _, jQuery, Utils, Configuration_1, UrlHelper_1, Backbone) {
    "use strict";
    // @module Facets
    ...
    var FacetsTranslator = function (facets, options, configuration, category) {
        // Enforces new
        ...
        var facets_data = Configuration_1.Configuration.get('siteSettings.facetfield');
        ...
        facets_to_include = _.union(facets_to_include, _.pluck(Configuration_1.Configuration.get('facets'), 'id'));
        ...
        if (_.isBoolean(category) && category) {
            var index_1 = facets.length;
            var facetsToInclude = this.facetsToInclude.slice(0);
            var facets_delimiter_1 = this.configuration.facetDelimiters.betweenDifferentFacets;
            var facet_value_delimiter_1 = this.configuration.facetDelimiters.betweenFacetNameAndValue;
            _.each(facetsToInclude, function (facetname) {
                facetname = facets_delimiter_1 + facetname + facet_value_delimiter_1;
            ...
            var categoryUrl = facets.substring(0, index_1);
            facets = facets.substring(index_1);
            if (categoryUrl[0] !== '/') {
            ...
            if (facets && facets[0] === facets_delimiter_1) {
    ...
    _.extend(FacetsTranslator.prototype, {
        ...
        parseUrl: function parseUrl(url) {
            ...
            var facets = facets_n_options[0] && facets_n_options[0] !== this.configuration.fallbackUrl
                ? facets_n_options[0]
                : '';
            ...
            var options_tokens = options.split(new RegExp('[\\' +
            ...
            while (options_tokens.length > 0) {
                ...
                var option_value = options_tokens.shift();
                var option_is_facet = _.contains(this.facetsToInclude, option_name);
        ...
        */
        ...
        getFacetConfig: function getFacetConfig(name) {
            ...
            result =
                result ||
                    _.find(this.configuration.facets, function (facet) {
                        return facet.url === name;
                    });
            return (result ||
                _.extend({ id: name, name: name, url: name, isParameter: undefined }, this.defaultFacetConfig));
        },
        // @method getUrl
        ...
        getUrl: function getUrl() {
            ...
            if (facets_seo_limits.numberOfFacetsGroups &&
            ...
            for (var i = 0; i < sorted_facets.length; i++) {
                ...
                switch (facet.config.behavior) {
                    ...
                    case 'multi':
                        ...
                        if (facets_seo_limits.numberOfFacetsValues &&
                ...
                if (self.facetIsParameter(name_1)) {
                    facets_as_options.push({ facetName: name_1, facetValue: value });
                }
                else {
                    ...
                    url += name_1 + self.configuration.facetDelimiters.betweenFacetNameAndValue + value;
                }
            }
            url = url !== '' ? url : '/' + this.configuration.fallbackUrl;
            // Encodes the Options
            var tmp_options = {};
            var separator = this.configuration.facetDelimiters.betweenOptionNameAndValue;
            ...
            var urlQuery = Backbone.history.fragment.split('?')[1];
            var parameters = _.pick(Utils.parseUrlOptions(urlQuery), function (value, key) {
                return !(key in _this.options) && _this.facetsToInclude.indexOf(key) === -1;
            });
            return UrlHelper_1.UrlHelper.fixUrl(Utils.addParamsToUrl(url, parameters));
        },
        ...
        getApiParams: function getApiParams() {
            ...
            _.each(this.facets, function (facet) {
                switch (facet.config.behavior) {
                    case 'range':
                        var value = typeof facet.value === 'object'
                            ? facet.value
                            : { from: 0, to: facet.value };
        ...
        cloneForFacetId: function cloneForFacetId(facet_id, facet_value) {
            ...
            var current_facet = _.find(facets, function (facet) {
                return facet.id === facet_id;
            });
            if (current_facet) {
        ...
        cloneForOptions: function cloneForOptions(object) {
            var facets = _.toArray(jQuery.extend(true, {}, this.facets));
            var options = jQuery.extend(true, {}, this.options, object);
            return new FacetsTranslator(facets, options, this.configuration, this.categoryUrl);
        },
...
//# sourceMappingURL=Facets.Translator.js.map
