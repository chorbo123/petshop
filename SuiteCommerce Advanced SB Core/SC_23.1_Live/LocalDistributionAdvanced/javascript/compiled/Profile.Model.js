...
*/
define("Profile.Model", ["require", "exports", "underscore", "Utils", "Configuration", "Address.Collection", "PaymentMethod.Collection", "PaymentInstrumentACH.Collection", "Singleton", "Backbone", "bignumber", "jQuery"], function (require, exports, _, Utils, Configuration_1, Address_Collection_1, PaymentMethod_Collection_1, PaymentInstrumentACH_Collection_1, Singleton, Backbone, BigNumber, jQuery) {
    "use strict";
    ...
    exports.ProfileModel = Backbone.Model.extend({
        ...
        initialize: function initialize(attributes) {
            ...
            this.set('paymentmethodsach', (attributes && attributes.paymentmethodsach) || new PaymentInstrumentACH_Collection_1.PaymentInstrumentACHCollection());
            this.on('change:paymentmethodsach', function (model, paymentMethodsACH) {
                // paymentMethodsACH is type any
                // for that reason PaymentInstrumentACHCollection (Typescript) is not working fine
                // we are using the reset method of the Collection class to make sure
                // that we are using the correct Model (PaymentInstrumentACHModel)
                if (Configuration_1.Configuration.get('checkoutApp.isAchEnabledInCheckout') ||
                    Configuration_1.Configuration.get('paymentInstrumentACHEnabled')) {
                    var paymentInstrumentACHCollection = new PaymentInstrumentACH_Collection_1.PaymentInstrumentACHCollection();
                    paymentInstrumentACHCollection.reset(paymentMethodsACH);
                    model.set('paymentmethodsach', paymentInstrumentACHCollection, {
                        silent: true
                    });
                }
            });
        ...
        getSearchApiUrl: function () {
            ...
            if (Configuration_1.Configuration.get('isNewSearchApiEnabled') === true) {
