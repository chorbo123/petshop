...
import { OrderWizardModulePaymentMethodACH } from './OrderWizard.Module.PaymentMethod.ACH';
...
const OrderWizardModulePaymentMethodSelector: any = WizardStepModule.extend({
    ...
    initialize: function(options) {
        ...
        const isAchEnableForCurrentCustomer =
            Configuration.get('checkoutApp.isACHEnabledForAllCustomers') ||
        ...
        if (
            ...
            this.modules.push(this.getACHModule(OrderWizardModulePaymentMethodACH));
        }
    ...
    render: function() {
        ...
        if (this.isOthersModule(this.selectedModule.type)) {
            const { type } = this.selectedModule;
            const other_module: any = _.findWhere(this.modules, {
                type: 'others'
            });
