        ...
        steps: [
            {
                ...
                save: function(): Promise<void> {
                    const paymentMethod = this.wizard.model.get('paymentmethods').at(0);
                    if (paymentMethod) {
                    ...
                    const shipmethods = [];
                    const multishipmethods = this.wizard.model.get('multishipmethods');
                    if (!_.isEmpty(multishipmethods)) {
                        _.each(multishipmethods, (shipmethodsCollection: any) => {
                            let shipmethod = shipmethodsCollection.findWhere({
                                check: true
                            });
                    ...
                    if (SC.CONFIGURATION.isThreeDSecureEnabled) {
                        return this.wizard.model.submit().then(
                            (): Promise<void> => {
                                ...
                                if (isPaymentAuthenticationRequired(confirmation)) {
                                    ...
                                    promise.rejectWith(this.wizard);
                                }
                                return promise;
                            }
                        );
