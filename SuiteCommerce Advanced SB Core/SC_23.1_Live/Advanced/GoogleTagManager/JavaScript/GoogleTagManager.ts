...
const GoogleTagManager: any = {
    ...
    // @return {Void}
    trackAddPaymentInfo: function({ cart, paymentType }) {
        if (cart) {
            ...
            cart.get('lines').each(function(model: any, index) {
    ...
    trackAddShippingInfo: function({ shipmethods, cart }) {
        if (shipmethods.length) {
            const eventName = 'add_shipping_info';
            const shipmethodsNames = _.map(shipmethods, (shipmethod: any) => {
            ...
            lines.each(function(line: any) {
                const item = line.get('item');
                eventData.ecommerce.items.push({
                    item_name: item.get('_name'),
                    item_id: item.get('itemid'),
                    price: line.get('rate'),
                    item_variant: _.map(line.get('options').toJSON(), function(option: any) {
            ...
            // Triggers a Backbone.Event so others can subscribe to this event and add/replace
            // data before being sent to Google Tag Manager
            Tracker.trigger(eventName, { shipmethods, lines });
            this.pushData(eventData);
        }
        return this;
    },

    trackRefund: function({ requestReturn }) {
        const { summary, lines } = requestReturn;
        const eventName = 'refund';
        const currency = SC.ENVIRONMENT.currencyCodeSpecifiedOnUrl;
        const coupon = _.pluck(requestReturn.promocodes, 'code').join(',');
        const eventData = {
            event: eventName,
            ecommerce: {
                currency,
        ...
        lines.forEach(line => {
            ...
            eventData.ecommerce.items.push({
                ...
                item_variant: _.map(line.options, function(option: any) {
                    return !!option.value && option.value.label;
                }).join(', '),
                price: line.rate,
        ...
        Tracker.trigger(eventName, { requestReturn });
        this.pushData(eventData);
        return this;
    },
    ...
    trackRemoveFromCart: function(line, quantity) {
        if (line) {
            const selectedOptions = line.get('options').filter(function(option) {
                return option.get('value') && option.get('value').label;
            });

            const item = line.get('item');
            const price = (item.get('_price') && item.get('_price').toFixed(2)) || 0.0;
                    ...
                    items: [
                        {
                            ...
                            name: item.get('_name'),
                            price: price,
                            variant: _.map(selectedOptions, function(option: any) {
                                return option.get('value').label;
                            }).join(', '),
                            category: this.getCategory(),
                            quantity: quantity
    ...
    trackProductView: function(product) {
        ...
        const itemList = item.get('list') || (result ? result.list : '');
        const eventData = {
            event: eventName,
            ecommerce: {
                detail: {
    ...
    loadScript: function() {
        return jQuery.getScript(
            `//www.googletagmanager.com/gtm.js?id=${this.configuration.id}&l=${
