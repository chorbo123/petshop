...
const LocationVenueDetailsView: any = BackboneView.extend({
    ...
    setOpeningHours: function setOpeningHours() {
        ...
        _.each(opening_hours, function(row: any) {
            const hours = `${row.starttime} ${Utils.translate('to').toLowerCase()} ${row.endtime}`;
            const days_list = [
                row.monday === 'T',
                row.tuesday === 'T',
                row.wednesday === 'T',
            ...
            if (sequence_active) {
                days = `${sequence_days[0]} ${Utils.translate('to').toLowerCase()} ${
            ...
            _.each(discontinuous_days, function(discontinuous_day, key) {
                if (key === 0 && !sequence_active) {
                    ...
                    days = `${days} ${Utils.translate(
                        'and',
                        'This "and" is used when enumerating days like: Monday, Wednesday and Friday. Prior to the last day enumerated we add the "and" word'
                    )} ${discontinuous_day}`;
                } else {
