...
const InvoiceCollection: any = Backbone.Collection.extend({
    ...
    // @method initialize
    initialize: function(models, options): void {
    ...
    update: function(options): void {
        if (this.options.status === 'makeAPayment') {
            ...
            Backbone.history.navigate(Backbone.history.fragment, { trigger: true, replace: true });
        }
    },
