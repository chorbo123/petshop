...
define('Invoice.Model', [
    ...
    return TransactionModel.extend({
        ...
        postList: function() {
            ...
            const payments = this.getPaymentStatus(invoice_ids);
        ...
        getPaymentStatus: function(invoiceIds) {
            const payments = [];
            const filters = [
                new nlobjSearchFilter('appliedtotransaction', null, 'anyof', invoiceIds)
            ];
            const columns = [
            ...
            const searchResult = Application.getAllSearchResults(
                'customerpayment',
                filters,
                columns
            );

            _.each(searchResult, payment => {
                const internalId = payment.getValue('appliedtotransaction');
                if (payments[internalId] === undefined) {
                    payments[internalId] = payment.getValue('status');
                }
            });
        ...
        postGet: function() {
            ...
            const payments = this.getPaymentStatus([this.result.internalid]);

            if (!_.isEmpty(payments)) {
                this.result.payment = { status: payments[this.result.internalid] };
            }
        },
