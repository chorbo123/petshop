...
export = BackboneView.extend({
    ...
    updateItemQuantity: function updateItemQuantity(e) {
        ...
        if (!validInput || options.quantity) {
            const newQuantity = parseInt(options.quantity, 10);
            ...
            const currentQuantity = parseInt(line.get('quantity'), 10);

            let newValidQuantity = newQuantity >= minQuantity ? newQuantity : minQuantity;
            newValidQuantity =
            ...
            if (newQuantity !== currentQuantity) {
                ...
                if (!invalid) {
                    const difference = line.get('quantity') - currentQuantity;
                    ...
                    update_promise
                        .fail(function(jqXhr) {
                            ...
                            line.set('quantity', currentQuantity);
                            self.render();
                        })
                        .always(function() {
