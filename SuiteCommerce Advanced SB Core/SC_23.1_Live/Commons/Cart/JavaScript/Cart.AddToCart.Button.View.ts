        ...
        try {
            setTimeout(function() {
                for (var i = 0; i < document.getElementsByClassName('bx-pager-link').length; i++) {
                    var activeImgOptionURL = (<HTMLImageElement>(
                        document.getElementsByClassName('bx-pager-link')[i].children[0]
                    )).currentSrc.split('?')[0];
                    var mainImg = document.getElementsByClassName(
                    ...
                    if (mainImgSplitURL[0] != 'src="' + activeImgOptionURL) {
                        ...
                        document.getElementsByClassName('product-details-image-gallery-container')[
                            i + 1
                        ].innerHTML = newImgSrc;
                    }
                }
            ...
            console.warn(
                'The Zoom bug fix is throwing an error (Cart.AddToCart.Button.View): ' +
                    JSON.stringify(e)
            );
        }
        e.preventDefault();
