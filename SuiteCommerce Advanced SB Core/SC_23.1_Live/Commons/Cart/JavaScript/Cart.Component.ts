...
export = function CartComponentGenerator(application) {
            ...
            try {
                ...
                this.serialize = this.serialize.pipe(function() {
                    ...
                    try {
                        ...
                        const oldLines = live_order_model.get('lines').toJSON();

                        live_order_model
                            .addLines(lines)
                            .done(function(result) {
                                const oldLinesById: _.Dictionary<{ quantity: number }> = _.indexBy(
                                    oldLines,
                                    'internalid'
                                );

                                const newLines = _.filter(
                                    result.lines,
                                    ({ internalid, quantity }) => {
                                        return (
                                            !oldLinesById[internalid] ||
                                            oldLinesById[internalid].quantity !== quantity
                                        );
                                    }
                                );

                                deferred.resolve(_.pluck(newLines, 'internalid'));
                            })
                            .fail(function(error) {
                                deferred.reject(error);
                            })
    ...
    live_order_model.cancelableOn('after:LiveOrder.addLines', function(oldLines, currentLines) {
        ...
        const oldLinesById: _.Dictionary<{ quantity: number }> = _.indexBy(oldLines, 'internalid');

        const newLines = _.filter(currentLines, ({ internalid, quantity }) => {
            return !oldLinesById[internalid] || oldLinesById[internalid].quantity !== quantity;
        });

        const linesDeferred = _.map(newLines, function(line: any) {
