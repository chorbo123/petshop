...
define('Profile.Model', [
    ...
    'SC.Models.Init',
    'PaymentMethod.Model',
    'PaymentInstrumentACH.Model',
    'Configuration',
    'Utils'
], function(SCModel, ModelsInit, PaymentMethodModel, PaymentMethodModelACH, Configuration, Utils) {
    return SCModel.extend({
        ...
        get: function() {
            ...
            if (Utils.isFeatureEnabled('personalized_catalog_views')) {
                profile.segments = [];
                const customerSegments = ModelsInit.customer.getCustomerSegments();
                const customerSegmentsIds = [];
                _.each(customerSegments, function(customerSegment) {
                    customerSegmentsIds.push(customerSegment.id);
                });

                const allCustomerSegments = ModelsInit.session.getAllCustomerSegments();
                _.each(allCustomerSegments, function(allCustomerSegment) {
                    if (customerSegmentsIds.indexOf(allCustomerSegment.id) !== -1) {
        ...
        update: function(data) {
            ...
            if (!data.skipLoginDontUpdateProfile) {
                this.validation.phone = function(value) {
                    if (value) {
            ...
            if (
                !(
                    this.currentSettings.companyname === '' ||
