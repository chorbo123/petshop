...
const ItemModel = BackboneCachedModel.extend({
    ...
    getDefaultPrice: function getDefaultPrice(details_object) {
        ...
        return {
            ...
            price: _.isNumber(details_object.onlinecustomerprice)
                ? details_object.onlinecustomerprice
                : this.get('onlinecustomerprice'),
            // @property {String} price_formatted
            price_formatted: _.isNumber(details_object.onlinecustomerprice)
                ? details_object.onlinecustomerprice_formatted
                : Utils.formatCurrency(this.get('onlinecustomerprice'))
        };
        // @class Item.Model
    },
    ...
    // @method getStrikeThroughPrice
    // Returns Strike through price
    // taking in consideration the OnlineCustomerPrice and Configured Default price level.
    getStrikeThroughPrice: function getStrikeThroughPrice(): string | null {
        if (!this._showStrikeThrough()) {
            return null;
        }

        const defaultPriceLevel: string = SC.CONFIGURATION.priceLevel.default;
        const defaultItemPrice = this.get(`${defaultPriceLevel}`);
        const defaultItemPriceFormatted = this.get(`${defaultPriceLevel}_formatted`);

        const matrixChildren = this.get('_matrixChilds') ? this.get('_matrixChilds').models : [];

        if (matrixChildren.length) {
            return this._getStrikeThroughForMatrixItems(matrixChildren);
        }

        const itemPrice = this.getPrice();
        return itemPrice.price < defaultItemPrice ? defaultItemPriceFormatted : null;
    },

    _showStrikeThrough: function showStrikeThrough(): boolean {
        ...
        return priceLevelEnabled && this.get(`${defaultPriceLevel}`);
    },

    getComparePrice: function getComparePrice(price, strikeThroughPrice): string {
        let comparePrice = '';
        if (price.compare_price > price.price) {
            comparePrice = price.compare_price_formatted;
        } else if (strikeThroughPrice) {
            comparePrice = strikeThroughPrice;
        }

        return comparePrice;
    },

    _getStrikeThroughForMatrixItems: function _getStrikeThroughForMatrixItems(
        matrixChildren
    ): string | null {
        ...
        _.each(matrixChildren, function(child: any) {
            childrenPrices.push({
                price: child.get(`${defaultPriceLevel}`),
                priceFormatted: child.get(`${defaultPriceLevel}_formatted`)
            });
        });

        const childrenPricesValues = _.pluck(childrenPrices, 'price');
        const minValue = _.min(childrenPricesValues);
        const maxValue = _.max(childrenPricesValues);

        const minPrice = _.where(childrenPrices, { price: minValue })[0];
        const maxPrice = _.where(childrenPrices, { price: maxValue })[0];

        if (minPrice === maxPrice) {
            return minPrice < defaultItemPrice ? minPrice.priceFormatted : null;
        }

        return Utils.translate('$(0) to $(1)', minPrice.priceFormatted, maxPrice.priceFormatted);
    }
});
