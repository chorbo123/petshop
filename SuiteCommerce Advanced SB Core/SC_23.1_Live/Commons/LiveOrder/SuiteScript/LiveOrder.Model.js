...
define('LiveOrder.Model', [
    ...
    return SCModel.extend({
        ...
        // @property {Boolean} isCardHolderAuthenticationSupported
        isCardHolderAuthenticationSupported: false,

        // @method get
        // @param {Boolean} threedsecure Received only in 3D Secure 2nd step
        ...
        get: function get(threedsecure) {
            ...
            if (
                ...
                this.authenticationOptions = {
                    ...
                    paymentOptionCsc: result.paymentmethods[0].creditcard.ccsecuritycode,
                    paymentProcessingProfile: paymentMethod.key.split(',')[1],
                    notificationURL: `${notificationURL}?n=${site_id}&type=${1}`
                };
            }
                    ...
                    for (let i = 0; i < data_payments.length; i++) {
                        ...
                        switch (data_payment.type) {
                            ...
                            case 'paypal':
                                ...
                                break;
                            case 'ACH':
                                filter.routingnumber = data_payment.routingnumber;
                                filter.entity = data_payment.entity;
                                filter.limit = data_payment.limit;
                                filter.accounttype = data_payment.accounttype;
                                filter.bankname = data_payment.bankname;
                                break;
        ...
        submit: function submit(threedsecure) {
            ...
            this.isCardHolderAuthenticationSupported = CardHolderAuthenticationUtils.isCHASupported(
                ModelsInit.order.getPayment()
            );
            ...
            if (
                ...
                ModelsInit.order.setCardHolderAuthentication(
                    ModelsInit.context.getSessionObject('cardHolderAuthenticationRecordId')
                );
            }
        ...
        getPaymentMethods: function getPaymentMethods(order_fields) {
            ...
            } else if (payment && payment.paymentterms === 'Ach') {
                const { ach } = payment;
                paymentmethods.push({
                    internalid: ach.internalid,
                    routingnumber: ach.routingnumber,
                    limit: ach.limit,
                    bankname: ach.bankname,
                    type: 'ach',
                    paymentmethod: {
                        internalid: ach.paymentmethod.internalid,
                        iscardholderauthenticationsupported:
                            ach.paymentmethod.iscardholderauthenticationsupported,
                        creditcardtoken: ach.paymentmethod.creditcardtoken,
                        isexternal: ach.paymentmethod.isexternal,
                        merchantid: ach.paymentmethod.merchantid,
                        name: ach.paymentmethod.name,
                        imagesrc: ach.paymentmethod.imagesrc,
                        ispaypal: ach.paymentmethod.ispaypal,
                        creditcard: ach.paymentmethod.creditcard,
                        key: ach.paymentmethod.key,
                        isautomatedclearinghouse: ach.paymentmethod.isautomatedclearinghouse
                    },
                    internal: {
                        name: ach.mask,
                        limit: ach.limit
        ...
        process3DSecure: function process3DSecure() {
            ...
            if (this.isCardHolderAuthenticationSupported) {
                return CardHolderAuthentication.process3DSecure(this.authenticationOptions, this);
            }

            // 3DS 1
            const orderHandlerUrl = CardHolderAuthenticationUtils.getThreeDSecurePageUrl(
                ModelsInit
            );
            ...
            if (confirmation.statuscode === 'error') {
                ...
                if (confirmation.reasoncode === this.CHA_STATUS.ERR_WS_REQ_PAYMENT_AUTHORIZATION) {
                    return confirmation;
                }
