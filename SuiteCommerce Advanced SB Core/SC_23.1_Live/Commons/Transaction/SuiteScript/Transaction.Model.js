    ...
    return SCModel.extend({
        ...
        list: function(data) {
            ...
            this.filters = {
                ...
                // Remove pending approval invoices
                status_operator: 'and',
                status: ['status', 'NONEOF', 'CustInvc:D'] // CustInvc:D = pendingApproval status
            };
            ...
            for (let i = 1; i <= this.record.getLineItemCount('promotions'); i++) {
                if (
                    ...
                    this.result.promocodes.push({
                        ...
                        isvalid:
                            this.record.getLineItemValue('promotions', 'promotionisvalid', i) ===
                            'T',
        ...
        getRecordPaymentMethod: function() {
            ...
            if (ccnumber) {
                ...
                if (paymentmethod.type === 'external_checkout') {
                    paymentmethod.id = this.record.getFieldValue('paymentoption');
                } else if (paymentmethod.type === 'internal') {
                    const ach = PaymentInstrumentModel.get(
        ...
        getLines: function() {
            ...
            for (let i = 1; i <= this.record.getLineItemCount('item'); i++) {
                if (
                    ...
                    line_id = `${
                        self.result.internalid === 'tempid'
                            ? self.result.quote_id
                            : self.result.internalid
                    }_${discline}`;
                    ...
                    this.result.lines[line_id] = this.result.lines[line_id]
                    ...
                    this.result.lines[line_id] = {
                        ...
                        isfulfillable:
                            this.record.getLineItemValue('item', 'fulfillable', i) === 'T',
        ...
        loadItemsWithStoreItem: function(items_to_preload) {
            ...
            _.each(this.result.lines, line => {
                if (line.item) {
                    let item = self.storeItem.getPreloadedItem(line.item);

                    if (item && item.ispricevisible === false) {
                        item = this.removePrices(item);
                    }
        ...
        getAddressInternalId: function(address) {
            const address_internalid = `${address.country || ''}-${address.state ||
