        ...
        if (isExtensible) {
            if (process.is_SCA_devTools) {
                ...
                const name = path.relative(
                    dest,
                    path.join(
                        package_manager.getNonManageResourcesPath(),
                        `${exportFile}-templates.js`
                    )
                );
            ...
            entryPoint.output[0].patterns = [
                '^((?!Handlebars\\.CompilerNameLookup\\.js)(?!javascript-libs\\.js).)*$'
            ];
        } else {
            entryPoint.names.unshift('javascript-libs.js');
...
gulp.task('libs', function libs() {
    ...
    const bundlerConfig = {
        ...
        paths: { Handlebars: 'handlebars.runtime-v4.7.7.js' }
    };
    /* eslint-enable */
    return gulp
